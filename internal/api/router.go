...
// RegisterRoutes 注册 /api/v1 下的全部路由，并挂载 API Key 鉴权中间件。
func RegisterRoutes(h *server.Hertz, apiKey string, batchHandler *handler.BatchHandler, sessionHandler *handler.SessionHandler) {
	v1 := h.Group("/api/v1", middleware.APIKeyAuth(apiKey))
