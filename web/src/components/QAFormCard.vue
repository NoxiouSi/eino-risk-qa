...
async function handleFiles(question: QuestionItem, event: Event) {
  ...
  const remaining = question.max_submit_count - current.fileIds.length
  if (remaining <= 0 || files.length > remaining) {
    emit('update:drafts', {
      ...props.drafts,
      [question.question_key]: { ...current, error: `最多可提交 ${question.max_submit_count} 张图片` },
    })
    input.value = ''
    return
  }
    ...
    <div class="question-list">
      <article v-for="question in props.questions" :key="question.question_key" class="question-item">
        <div class="question-meta">
          ...
          <span v-if="question.answer_type !== 'text'" class="count">
            {{ question.required ? `可提交 ${question.min_submit_count}–${question.max_submit_count} 张` : `提交时可选 ${question.min_submit_count}–${question.max_submit_count} 张` }}
          </span>
        </div>
        <textarea
        ...
        <div v-else class="upload-area">
          <label class="upload-button" :class="{ disabled: props.disabled || props.generating || props.drafts[question.question_key]?.uploading || (props.drafts[question.question_key]?.fileIds.length ?? 0) >= question.max_submit_count }">
            <input type="file" accept="image/jpeg,image/png,image/webp" multiple :disabled="props.disabled || props.generating || (props.drafts[question.question_key]?.fileIds.length ?? 0) >= question.max_submit_count" @change="handleFiles(question, $event)" />
